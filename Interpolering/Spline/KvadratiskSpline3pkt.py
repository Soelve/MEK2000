"""Dette skriptet konstruerar ein kvadratisk spline for eit sett med tre punkt.
...
#
Mat = [[x[0]**2, x[0], 1, 0, 0, 0],    # Første seks: Interpolere punkta
       [x[1]**2, x[1], 1, 0, 0, 0],
       [0, 0, 0, x[1]**2, x[1], 1],
       [0, 0, 0, x[2]**2, x[2], 1],
       [2*x[1], 1, 0, -2*x[1], -1, 0],
       [1, 0, 0, 0, 0, 0]]             # Siste: p_1''(x) = 0
...
print(f'p_0(x) = {Coeff[0]:.2f} x**2 + {Coeff[1]:.2f} x + {Coeff[2]:.2f}')
print(f'p_1(x) = {Coeff[3]:.2f} x**2 + {Coeff[4]:.2f} x + {Coeff[5]:.2f}')
